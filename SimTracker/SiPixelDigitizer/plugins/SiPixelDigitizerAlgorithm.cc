...
void SiPixelDigitizerAlgorithm::induce_signal(std::vector<PSimHit>::const_iterator inputBegin,
  ...
  if (UseReweighting) {
    if (hit.processType() == 0) {
      ...
      std::vector<PSimHit>::const_iterator crSimHit;
      // As it is not the primary particle, check if the first hit in the same detId corresponds to the same simulated track
      if ((*inputBegin).trackId() == hit.trackId()) {
        ...
        for (std::vector<PSimHit>::const_iterator ssbegin = inputBegin; localIndex < hitIndex;
          ...
          if ((*ssbegin).trackId() == hit.trackId() && (*ssbegin).processType() == 0) {
            ...
            found_the_primary = true;
            break;
          }
        }
        if (!found_the_primary) {
          // we haven't found a hit associated to the same trackId --> use the 1st hit in the same detId then
          crSimHit = inputBegin;
          //  ReferenceIndex4CR=FirstHitIndex;
        }
      }
